...
def format_mbti_string(mbti_qualities: dict[str, int]) -> str:
    ...
    for quality, score in mbti_qualities.items():
        if score != 0:  # Only include qualities with non-zero scores
            hebrew_quality = MBTI_QUALITIES_HEBREW.get(
                quality, quality
            )  # Use English if Hebrew not found
...
def collect_midzone_qualities(file_path: str) -> list[str]:
    ...
    try:
        ...
        # Updated pattern to match "midzone" and the following word
        pattern = r"\(midzone\)[-\s]*(\w+)"

        # Find all matches
        matches = re.findall(pattern, content, re.IGNORECASE | re.MULTILINE)

        # Add matches to the list
        midzone_qualities.extend(matches)
...
if __name__ == "__main__":
    ...
    midzone = collect_midzone_qualities(file_path)
    print("Midzone qualities:", midzone)
