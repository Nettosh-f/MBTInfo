...
from .utilsAI import (
    ...
    format_page_10_content,
    get_all_info,
    get_formatted_type_qualities,
)
...
async def create_translated_pdf(input_file, output_file):
    ...
    try:
        ...
        with open(extracted_text_path, encoding="utf-8") as f:
            ...
            translated_file_path = os.path.join(
                output_dir, f"{base_name}_translated_raw.txt"
            )
    ...
    fixed_text_config = fixed_text_data(
        mbti_info, mbti_type_qualities, formatted_page_10
    )
    fixes_translated_text_path = os.path.join(output_dir, f"{base_name}_fixed.txt")
    insert_fixed_text(
        translated_file_path, fixes_translated_text_path, fixed_text_config
    )
